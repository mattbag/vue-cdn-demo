    ...
    function(FabricWindow, $timeout, $window, FabricCanvas, FabricDirtyStatus) {
      ...
      return function(options) {
        ...
        self.addObjectToCanvas = function(object) {
          // console.log(self.canvasOriginalWidth);
          ...
          // object.scaleToWidth(self.canvasOriginalWidth);
          canvas.add(object);
        ...
        self.select_on_canvas = function(object) {
          // console.log(object);
          canvas.setActiveObject(object);
        };
        //
        // scaling image if bigger than canvas
        // ===================================================================
        self.scale_to_fit = function(_image_object) {
          // console.log(_image_object);
          var ih = _image_object._originalElement.naturalHeight;
          var iw = _image_object._originalElement.naturalWidth;

          var width_ratio = canvas.width / iw;
          var height_ratio = canvas.height / ih;

          if (ih > canvas.height || iw > canvas.width) {
            console.log('time to scale');
            if (width_ratio > height_ratio) {
              _image_object.scaleToHeight(canvas.height);
            } else {
              _image_object.scaleToWidth(canvas.width);
            }
        ...
        self.addImage = function(imageURL, filename) {
          ...
          fabric.Image.fromURL(imageURL, function(object) {
            ...
            // adding filename e src for layers
            object.filename = filename;
            object.fileSrc = imageURL;

            // self.scale_to_fit(object);
            // canvas.centerObject(object);
            // object.setCoords();
            object.set({
              originX: 'center',
              originY: 'center',
              scaleX: self.overlay_object.scaleX,
              scaleY: self.overlay_object.scaleY,
            });
            if(object.width * object.scaleX > canvas.width){
              console.log('image bigger');
              self.scale_to_fit(object);
              object.originalScaleX = 1;
              object.originalScaleY = 1;
            }else{
              object.originalScaleX = self.overlay_object.scaleX;
              object.originalScaleY = self.overlay_object.scaleY;
            }
            // console.log(object.originalScaleX);
            // console.log('set the if and set scale for small prods');
            // console.log(object);
            object.applyFilters(canvas.renderAll.bind(canvas));

            self.addObjectToCanvas(object);
            self.sendToBack(object);
            // console.log(object);
            self.image_layers.push(object);
            // console.log(self.image_layers);
        ...
        // ================================
        self.addOverlay = function(overlayURL) {
          // console.log('url '+ imageURL);
          fabric.Image.fromURL(overlayURL, function(object) {
            object.id = self.createId();
            self.overlay_object = object;

            self.scale_to_fit(self.overlay_object);
            canvas.centerObject(self.overlay_object);

            // canvas.setOverlayImage(object, canvas.renderAll.bind(canvas));
            // console.log(object);


            self.addObjectToCanvas(self.overlay_object);
            // console.log(self.overlay_object);
            canvas.moveTo(self.overlay_object,20);
            self.clip_to_overlay();
            self.toggleLockActiveObject(self.overlay_object);
            self.overlay_object.selectable = false;
            self.overlay_object.evented = false;
            // self.deselectActiveObject();
        ...
        //
        // function to clip the canvas to the overlay object
        self.clip_to_overlay = function() {
          canvas.clipTo = function(ctx) {
            // console.log(self.overlay_object);
            ctx.rect(self.overlay_object.left, self.overlay_object.top, self.overlay_object.width * self.overlay_object.scaleX, self.overlay_object.height * self.overlay_object.scaleY, true);
          };
        }
        ...
        // Active Object
        // ==============================================================
        self.returnActiveObject = function() {
        ...
        self.deleteActiveObject = function(activeObject) {
          // var activeObject = self.selectActiveObject();
          if (!activeObject) {
            return;
          }
          ...
          if (activeObject.type === 'image') {
            console.log('image object removing index:');
            console.log(self.image_layers.indexOf(activeObject));
            var object_index = self.image_layers.indexOf(activeObject);
            self.image_layers.splice(object_index, 1);
          } else if (activeObject.type === 'text') {
            console.log('text object: removing index:');
            console.log(self.text_layers.indexOf(activeObject));
            var object_index = self.text_layers.indexOf(activeObject);
            self.text_layers.splice(object_index, 1);
            // console.log('------------------------------------');
            // console.log(self.text_layers);
            // console.log('------------------------------------');
          }
        ...
        self.loadJSON = function(json) {
          ...
          canvas.loadFromJSON(json, function() {
           $timeout(function() {self.setLoading(false);if (!self.editable) {self.disableEditing();}self.render();});
          });
        };
        ...
        self.download = function(name) {
          // remove the clipping to the frame for saving the image
          // canvas.clipTo = null;
          ...
          // link.click();
          window.open(self.getCanvasData());
          // self.clip_to_overlay();
          self.render();
        ...
        //  download 2
        self.downloadCanvasImage = function() {
          canvas.deactivateAll().renderAll();
          ...
          var w = open();
          w.document.title = 'Save the composition';
          w.document.body.appendChild(a);

        };

        self.setCanvasImageData = "";
          //  download 2
        self.setCanvasImage = function () {
            canvas.deactivateAll().renderAll();
            self.setCanvasImageData = canvas.toDataURL({
                format: 'png',
                multiplier: 1
            });
        };
        //
        // ====================== generate pdf - needs jspdf

        self.generate_pdf = function() {
          canvas.deactivateAll().renderAll();
          ...
          try {
            ...
            pdf.addImage(imgData, 'JPEG', 0, 0, 297 * .7, 210 * .83);
            var namefile = 'custom_filename';
            if (namefile != null) {
        ...
        self.convert_SVG = function (value){
